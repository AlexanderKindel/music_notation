...
struct SliceRodLog
{
    ...
    //The nth entry represents the length of the rod that begins on the nth slice and ends on this
    //slice.
    int32_t*uz_rod_lengths;
    struct Rational whole_notes_long_of_min_slice_duration;
    float uz_max_space_height_of_min_slice_duration;
};
...
void add_rod_log(struct Project*project, struct SliceIter*slice_iter, uint32_t log_count)
{
    ...
    rod_log->whole_notes_long_of_min_slice_duration = (struct Rational) { &TWO, &ONE };
    rod_log->uz_max_space_height_of_min_slice_duration = 0.0;
}
    ...
    struct Project*project)
{
    ...
    for (size_t spring_index = spring_count; spring_index-- > 0;)
    {
        ...
        struct SliceRodLog*log_of_rod_at_spring_end = rod_logs + spring_index + 1;
        uint32_t node_index = log_of_rod_at_spring_end->slice->first_object_address_node_index;
        do
        {
            ...
            if (!object_after_iter->is_hidden)
            {
                ...
                while (true)
                {
                    do
                    {
                        decrement_page_element_iter(&object_iter.base, &project->page_pool,
                            sizeof(struct Object));
                    } while (object_iter.object->is_hidden);
                        ...
                        uz_rod_length += float_round(uz_space_height * spacer) +
                            (MAX(get_string_width(device_context, uz_font_set.full_size,
                                ...
                                get_string_width(device_context, uz_font_set.full_size,
                                    strings.denominator_string,
                                    strings.denominator_string_length)));
                    }
                    };
                    if (object_iter.object->slice_address)
                    {
                        void*stack_b_math_savepoint = project->stack_b.cursor;
                        uz_rod_length -=
                            uz_get_default_distance_from_object_origin_to_slice(uz_space_height,
                                object_iter.object);
                        struct Rational whole_notes_long_of_slice_duration = { &ZERO, &ONE };
                        struct SliceIter staff_slice_iter;
                        initialize_page_element_iter(&staff_slice_iter.base,
                            log_of_rod_at_spring_end->slice, sizeof(struct Slice));
                        do
                        {
                            ...
                            add_rationals(&whole_notes_long_of_slice_duration,
                                ...
                                &project->stack_a);
                        } while (staff_slice_iter.slice->address !=
                            object_iter.object->slice_address);
                        log_of_rod_at_spring_end->uz_rod_lengths[
                            staff_slice_iter.slice->index_in_slice_range] =
                            MAX(log_of_rod_at_spring_end->uz_rod_lengths[
                                staff_slice_iter.slice->index_in_slice_range], uz_rod_length);
                        struct SliceRodLog*log_at_rod_start =
                            rod_logs + staff_slice_iter.slice->index_in_slice_range;
                        int8_t comparison = compare_rationals(&whole_notes_long_of_slice_duration,
                            &log_at_rod_start->whole_notes_long_of_min_slice_duration,
                            &project->stack_a);
                        if (comparison < 0)
                        {
                            log_at_rod_start->whole_notes_long_of_min_slice_duration =
                                whole_notes_long_of_slice_duration;
                            log_at_rod_start->uz_max_space_height_of_min_slice_duration =
                                uz_space_height;
                        }
                        else
                        {
                            ...
                            {
                                log_at_rod_start->uz_max_space_height_of_min_slice_duration =
                                    MAX(log_at_rod_start->uz_max_space_height_of_min_slice_duration,
                                        uz_space_height);
                            }
                            project->stack_b.cursor = stack_b_math_savepoint;
                        }
        ...
        struct SliceRodLog*log_of_rod_at_spring_start = rod_logs + spring_index;
        int32_t uz_slice_duration_rod_length =
            uz_get_duration_width(&log_of_rod_at_spring_start->slice->whole_notes_long, project,
                log_of_rod_at_spring_start->uz_max_space_height_of_min_slice_duration);
        log_of_rod_at_spring_end->uz_rod_lengths[spring_index] =
            MAX(log_of_rod_at_spring_end->uz_rod_lengths[spring_index],
                uz_slice_duration_rod_length);
        ...
        else
        {
            ...
            confirmed_augmentation[spring_index] =
                log_of_rod_at_spring_end->uz_rod_lengths[spring_index];
            log_of_rod_at_spring_end->log_of_previous_joined_slice = log_of_rod_at_spring_start;
            for (size_t i = 0; i < spring_index; ++i)
            {
                log_of_rod_at_spring_end->uz_rod_lengths[i] -=
                    log_of_rod_at_spring_end->uz_rod_lengths[spring_index];
                log_of_rod_at_spring_start->uz_rod_lengths[i] =
                    MAX(log_of_rod_at_spring_start->uz_rod_lengths[i],
                        log_of_rod_at_spring_end->uz_rod_lengths[i]);
                log_of_rod_at_spring_end->uz_rod_lengths[i] = 0;
            }
            log_of_rod_at_spring_end->uz_rod_lengths[spring_index] = 0;
        }
    }
...
matrix_finalized:
    ...
    for (size_t log_index = 1; log_index <= spring_count; ++log_index)
    {
        struct SliceRodLog*log = rod_logs + log_index;
        for (size_t rod_index = 0; rod_index < log_index; ++rod_index)
        {
    ...
    if (unactivated_rod_count)
    {
        do
        {
            ...
            while (address_rod_index < unactivated_rod_count)
            {
                ...
                for (size_t i = address.rod_index_in_log; i < address.rod_log_index; ++i)
                {
                    tentative_matrix[0][i] = 1.0;
                }
                for (size_t i = 1; i < spring_count; ++i)
                {
                ...
                for (size_t i = 0; i < spring_count; ++i)
                {
                    ...
                    for (size_t k = i + 1; k < spring_count; ++k)
                    {
                        float scalar = tentative_matrix[k][i] / tentative_matrix[i][i];
                        for (size_t l = i; l < spring_count; ++l)
                        {
            ...
            if (uz_max_slice_range_width_at_which_rod_actives)
            {
                ...
                for (size_t i = address_of_rod_to_activate.rod_index_in_log;
